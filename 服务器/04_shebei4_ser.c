...
#include <time.h>
#define FILE_NAME "shebei4.txt"
/**************************************************************
 * 备注          :  宏定义区域
***************************************************************/
#define MQTT_ADDR "tcp://mqtt.yyzlab.com.cn"    //云平台地址
#define ClientID1 "ubunut_guo_2023018586852"           //标识id号，在同一个云平台下，id号不能冲突            
#define SUB_TOPIC1 "1688716903574/AIOTSIM2APP"  //订阅、发布主题
#define PUB_TOPIC1 "1688716903574/APP2AIOTSIM"
...
char lamp2_on[100]="{\"lamp2\":true,\"id\":0}";   //设备控制命令
char lamp2_off[100]="{\"lamp2\":false,\"id\":0}";
char lamp3_on[100]="{\"lamp3\":true,\"id\":0}";   //设备控制命令
char lamp3_off[100]="{\"lamp3\":false,\"id\":0}";
...
int tcp_server(void)
{
	...
	ser.sin_port = htons(5904);           //将主机序转成网络序
...
void *clint(void *arg)
{
	...
	while(1)
	{
        ...
        }else if(strcmp(recv_buf, "lamp2_on")==0)
        {
            mqtt_pub(mqtt_client,lamp2_on,PUB_TOPIC1);
        }else if(strcmp(recv_buf, "lamp2_off")==0)
        {
            mqtt_pub(mqtt_client,lamp2_off,PUB_TOPIC1);
        }else if(strcmp(recv_buf, "lamp3_on")==0)
        {
            mqtt_pub(mqtt_client,lamp3_on,PUB_TOPIC1);
        }else if(strcmp(recv_buf, "lamp3_off")==0)
        {
            mqtt_pub(mqtt_client,lamp3_off,PUB_TOPIC1);
        }
...
int recv_message(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
    ...
    // }
    sprintf(data_buf3, "{\"tem\":%f,\"hum\":%f,\"light\":%f}", tem, hum, light);
